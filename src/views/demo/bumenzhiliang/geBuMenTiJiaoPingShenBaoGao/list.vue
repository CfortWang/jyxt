...
<script>
import { queryPageList, remove } from '@/api/demo/bumenzhiliang/geBuMenTiJiaoPingShenBaoGao'
...
export default {
  ...
  data() {
    return {
      ...
      listConfig: {
        ...
        // 表格字段配置
        columns: [
          { prop: 'bianZhiNeiRong', label: '编制内容' }
          ,{ prop: 'gangWei', label: '岗位' }
  ...
  methods: {
    ...
    getSearcFormData() {
      ...
      where['Q^bian_zhi_bu_men_^SL']= this.orgId
    ...
    handleAction(command, position, selection, data) {
      switch (command) {
        ...
        case 'add':// 添加
          this.handleEdit()
          this.title = '添加t_gbmtjnr'
          break
        case 'edit':// 编辑
          ActionUtils.selectedRecord(selection).then((id) => {
            this.handleEdit(id)
            this.title = '编辑t_gbmtjnr'
          }).catch(() => { })
          break
        case 'detail':// 明细
          ActionUtils.selectedRecord(selection).then((id) => {

<template>
  <div class="main-container">
    ...
    <ibps-link
      ...
      ref="printResult"
      text="打印一览表"
      link="resolve([{event:'afterSubmit',logic:`resolve({openType:'dialog',url:'${options.reportPash}27实验室间对比和能力验证程序/SGJS-CX-27-03B 实验室间比对一览表.rpx&id_=${options.formData.id}'})` }])"
      show-type="button"
      text-type="fixed"
...
<script>
import { queryPageList, remove } from '@/api/demo/bumenzhiliang/shiYanShiJianBiDuiYiLanXiang'
  ...
  data() {
    return {
      ...
      listConfig: {
        ...
        searchForm: {
          forms: [
            { prop: 'Q^CAN_JIA_XIANG_MU_^SL', label: '参加项目名称' }
        ...
        // 表格字段配置
        columns: [
          { prop: 'canJiaXiangMu', label: '参加项目名称' }
          ,{ prop: 'zuZhiFang', label: '组织方' }
          ,{ prop: 'canJiaShiYanS', label: '参加实验室' }
          ,{ prop: 'canJiaRiQi', label: '参加日期' }
          ,{ prop: 'jieGuo', label: '结果' }
          ,{ prop: 'beiZhu', label: '备注' }
          ,{ prop: 'shiYanJianBiD', label: '试验间比对文件',slotName:"JiShuFuJian" }
        ],
        rowHandle: {
            actions: [
  ...
  methods: {
    ...
    handleAction(command, position, selection, data) {
      switch (command) {
        ...
        case 'add':// 添加
          this.handleEdit()
          this.title = '添加t_sysjbdylb'
          break
        case 'edit':// 编辑
          ActionUtils.selectedRecord(selection).then((id) => {
            this.handleEdit(id)
            this.title = '编辑t_sysjbdylb'
          }).catch(() => { })
          break
        case 'detail':// 明细
          ActionUtils.selectedRecord(selection).then((id) => {

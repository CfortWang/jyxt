<template>
    <div id="digital-flop">
        <!-- <template v-if="item.title != '委托案件'"> -->
        <div v-for="item in digitalFlopData" :key="item.title" class="digital-flop-item">
            <div class="digital-flop-title">{{ item.title }}</div>
            <div class="digital-flop">
                <dv-digital-flop :config="item.number" style="width: 80px; height: 50px; font-size: 18px" />
...
export default {
    ...
    methods: {
        createData() {
            ...
            staffTaskNum().then((res) => {
                console.log(res)
                // 将参数替换成对应参数
                if (res.state === 200 && res.variables.data) {
                    this.digitalFlopData = []
                    let digCont = res.variables.data
                    digCont = []

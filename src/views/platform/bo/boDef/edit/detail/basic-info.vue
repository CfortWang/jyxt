<template>
  <el-form ref="form" class="basic-info-form" :model="form" :rules="rules" label-width="140px" @submit.native.prevent>
    <el-row>
      <el-col :span="24">
      ...
      <el-col :span="24">
        <el-form-item prop="code">
          <span slot="label">
            编码<span v-if="id">：</span>
            <el-tooltip
              v-if="!id"
              class="item"
              effect="light"
              content="请统一编码值大写或者小写！"
              placement="bottom"
            >
      ...
      <el-col v-if="!isMaster" :span="24">
          <el-form-item  label="过滤字段:" prop="filterColumns">
           <el-select clearable v-model="form.filterColumns" :disabled="readonly" style="width:100%;">
              <el-option
                v-for="item in changeOptions"
                :key="item.fieldName"
                :label="item.name"
      ...
      <el-col v-if="!isMaster" :span="24">
        <el-form-item label="关系类型：" prop="relation">
          <el-select v-model="form.relation" :disabled="readonly||(!form.edit&&pass)||isCreateTable==='Y'" style="width:100%;">
            <el-option
...
export default {
  ...
  data() {
    ...
    const validateCode = (rule, value, callback) => {
      const reg = /^[a-zA-Z][a-zA-Z0-9_]{0,18}$/
      if (!reg.test(value)) {
  ...
  watch: {
    ...
    'form.code': {
      handler: function(val, oldVal) {
        ...
        if (this.form.edit) {
          // 校验[后端]
          if (this.form.status === '') {
            if ((reg.test(val) || regLetter.test(val)) && val.length < 17) {
    ...
    formData: {
      handler: function(val) {
        ...
        const rootIndex = this.changeOptions.findIndex(item => item.code === this.form.fk)
        this.form.pk = rootIndex > -1 ? this.form.pk : (this.changeOptions.length > 0 ? this.changeOptions[0].code : '')
      },
      deep: true
    },
    treeData: {
    ...
    changeOptions: { // 初始化下拉数据。
      handler: function(val, oldVal) {
        if (!val || this.$utils.isEmpty(val)) return

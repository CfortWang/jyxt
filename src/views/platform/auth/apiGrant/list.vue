...
<script>
import { queryPageList, doAuditBatch, remove, grant, passAllByRole } from '@/api/platform/auth/apiGrant'
...
export default {
        ...
        rowHandle: {
          actions: [{
            ...
            hidden: (row, index) => {
              ...
              if (this.$store.getters.isSuper && !this.dialog && this.$store.getters.userId !== row.createBy) return false
  ...
  computed: {
    flexHeight() {
      return this.dialog ? this.dialogHeight : this.height
    }
  },
  watch: {
    dialog: {
  ...
  methods: {
    ...
    getSearcFormData() {
      ...
      params['Q^GRANT_KEY_^S'] = this.searchClient || ''
      ...
      } else if (this.grantType === 'client' || this.grantType === 'role' || this.grantType === 'res') {
        params['Q^APP_KEY_^SL'] = this.searchAppKey
        params['Q^GRANT_TYPE_^S'] = this.grantType
        params['Q^GRANT_KEY_^S'] = this.searchClient
        this.defaultParams = { 'grantType': this.grantType, 'grantKey': this.clientKey }
      }
      if (this.$utils.isNotEmpty(this.storePagination)) {
        this.pagination.page = this.storePagination.page
        this.pagination.limit = this.storePagination.limit
      }
    ...
    handlePaginationChange(page) {
      ...
      this.storePagination.page = page.page
      this.storePagination.limit = page.limit
      this.loadData()
    },
    /**
     * 处理排序
    ...
    handleAction(command, position, selection, data) {
      switch (command) {
        ...
        case 'detail':// 明细
          ...
          break
        case 'passAllByRole':// 角色授权接口全部通过
          ActionUtils.removeRecord('0', '确定通过授权角色的全部接口?', true).then(() => {
            passAllByRole(this.getSearcFormData()).then(response => {
    ...
    handleConfirm(data) {
      ...
      grant({
        ...
        ActionUtils.saveSuccessMessage(response.message, (rtn) => {
          this.selectorApiVisible = !rtn
        })
        this.loadData()
      })
    },
    callback() {
      return this.selectorApiVisible
    },
    closeAll() {

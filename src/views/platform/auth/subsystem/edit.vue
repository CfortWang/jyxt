  ...
  >
    ...
    >
      ...
      <el-form-item label="标志(logo)类型:" prop="logoType">
        ...
        <el-tag v-else>{{ subsystem.logoType|optionsFilter(typeOptions,'label') }}</el-tag>
      </el-form-item>
      ...
      <el-form-item v-if="!subsystem.isLocal" label="子系统地址:" prop="homePage">
        <el-input v-if="!readonly" v-model="subsystem.homePage" />
        <span v-else>{{ subsystem.homePage }}</span>
        <div v-if="!readonly" class="ibps-help-block">首页的路径。例：http://www.bpmhome.cn/，也可以写好的vue页面</div>
      </el-form-item>

...
<script>
import { getMenuFavorites, sortFavorites, removeFavorites } from '@/api/platform/auth/resources'
  ...
  methods: {
    ...
    getSearcFormData() {
      return ActionUtils.formatParams(
        this.$refs['crud'] ? this.$refs['crud'].getSearcFormData() : { 'sign': 'personal', 'systemId': this.$store.getters.systemid },
        this.pagination,
        this.sorts)
    },
    /**
     ...
     */
    handleRemove(ids) {
      removeFavorites({ resourceIds: ids, sign: 'personal' }).then(response => {
     ...
     */
    handleSort(ids, sort) {
      sortFavorites({ resourceIds: ids, sort: sort, sign: 'personal' }).then(response => {

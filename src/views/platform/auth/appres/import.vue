...
<script>
import { importXml } from '@/api/platform/auth/appres'
  ...
  methods: {
    ...
    handleImport() {
      if (this.$utils.isEmpty(this.$refs.upload.uploadFiles)) {
        ActionUtils.saveErrorMessage('请选择 *.xml文件进行导入!')
        return
      }
      ...
      this.$confirm('是否覆盖原有资源？?', '提示', {
        ...
        if (this.$utils.isEmpty(file) || file.type !== 'text/xml') {
          ActionUtils.saveErrorMessage('请导入xml文件！')
        } else {
    ...
    // 提交保存数据
    saveData(file, appresId, systemId) {
      importXml(file, appresId, systemId).then(response => {

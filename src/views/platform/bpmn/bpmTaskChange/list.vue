<template>
	<div class="main-container">
		<ibps-crud ref="crud" :height="height" :data="listData" :displayField="title" :toolbars="listConfig.toolbars"
			...
			@action-event="handleAction" @column-link-click="handleLinkClick" @sort-change="handleSortChange"
			@pagination-change="handlePaginationChange" />
		<edit :id="editId" :title="title" :readonly="readonly" :visible="dialogFormVisible" @callback="search"
			@close="visible => dialogFormVisible = visible" />
		<bpmn-formrender :visible="formrenderDialogFormVisible" :instance-id="instanceId" :task-change-id="editId"
...
<script>
	import {
		queryPageList,
		setStatus
	} from '@/api/platform/bpmn/bpmTaskChange'
		...
		data() {
			return {
				...
				listConfig: {
					...
					searchForm: {
						forms: [{
								prop: 'Q^TASK_SUBJECT_^SL',
								label: '事项标题'
							},
							{
								prop: 'Q^TASK_NAME_^SL',
								label: '任务名称'
							},
							{
					...
					rowHandle: {
						actions: [{
							...
							hidden: function(row, index) {
								if (row.ownerId === this.$store.getters.userId && row.status === 'running')
									return false
								return true
							}
						}, {

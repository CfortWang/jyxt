  ...
  <div>
    ...
    <template slot="selectCont" slot-scope="scope">
      ...
      >
        ...
        <div class="div_test el-icon-s-order" slot="reference" >查阅</div>
        <!-- 弹框内容 -->
      ...
      <template slot="goodsFactory"  slot-scope="scope">
            <ibps-custom-dialog
                  :value="scope.row.goodsFactory"
                   template-key="gyspjb"
                  :disabled="true"
                  readonly-text="text"
                />
      ...
      <!-- 报告列的附件插槽 -->
      <template slot="reportSolt" slot-scope="scope">
            <ibps-attachment
...
export default {
    ...
    return {
      ...
      listConfig: {
        ...
        columns: [
          ...
          { prop: 'report', label: '报告' , width: 120,  fieldType: 'slot',slotName:"reportSolt"},
          { prop: 'tUser', label: '评定人' , width: 80},
          { prop: 'endTime', label: '评定时间', sortable: 'custom', dateFormat: 'yyyy-MM-dd', width:110 },
        ],
        rowHandle: {
          actions: [/* {
  ...
  methods: {
   ...
   getParenthesesStr(text) {
    ...
    clbqddpdkzcx(cont,data){
      // console.log('cont:\n',cont)
       for(let i in data){
        if(data[i].bizKey ==cont.id_) {
          data[i]["report"] = cont.shang_chuan_wen_j
        }
       }
        // resolve(true)
     },

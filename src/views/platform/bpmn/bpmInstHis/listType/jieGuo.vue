  ...
  <div>
    ...
    <template slot="selectCont" slot-scope="scope">
      ...
      <template slot="qualityControlSupervisorSlot"  slot-scope="scope">
          <ibps-user-selector
             :value="scope.row.qualityControlSupervisor"
             type="user"
             :multiple="false"
             :disabled="true"
             readonly-text="text"
...
export default {
  ...
  data() {
    return {
      ...
      responseData:"",
      dataConfig:{
        "zlkzpsjlqy":{t_zlkzpsjlqy:[],},    //质量控制评审记录
        "ndzlkzjhqy":{t_ndzlkzjh:[]},     //年度质量控制计划
      },
      listConfig: {
        toolbars: [
          { key: 'search' },
        ...
        columns: [
         /* { prop: 'subject', label: '任务标题', link: 'dialog', width: 250 }, */
          ...
          { prop: 'project', label: '质控项目' , width: 280},
          { prop: 'qualityControlSupervisor', label: '质量监督员' , width: 150, fieldType: 'slot',slotName:"qualityControlSupervisorSlot"},
          { prop: 'endTime', label: '质控完成时间', sortable: 'custom', dateFormat: 'yyyy-MM-dd', width:150 },
        ],
        rowHandle: {
          actions: [/* {
  ...
  methods: {
   ...
   getParenthesesStr(text) {
    ...
    /* 质控项目*/
    zlkzpsjlqy(cont,data){
        // console.log('质量控制评审记录: ',cont)
       for(let i in data){
        if(data[i].bizKey ==cont.id_) {
          data[i]["project"] = cont.zhi_kong_xiang_mu
          data[i]['qualityControlSupervisor'] = cont.zhi_kong_ren_yuan
        }
       }
        // resolve(true)
     },
     //年度质量控制计划
     ndzlkzjhqy(cont,data){
        // console.log('年度质量控制计划: ',cont)
        for(let i in data){
         if(data[i].bizKey ==cont.id_) {
           data[i]["project"] = cont.zhi_liang_kong_zh
           // data[i]['qualityControlSupervisor'] = cont.zhiKongRenYuan
         }
        }
        // resolve(true)
      },

  ...
  <div>
    ...
    <template slot="selectCont" slot-scope="scope">
      ...
      <template slot="facilitySlot"  slot-scope="scope">
        ...
        <template slot="facilityFactor"  slot-scope="scope">
              <ibps-custom-dialog
                    :value="scope.row.facilityFactor"
                     template-key="gysdhk"
                    :disabled="true"
                    readonly-text="text"
                  />
        ...
        <!-- <template slot="facilityNumberSlot"  slot-scope="scope">
              <ibps-custom-dialog
                    :value="scope.row.facilityNumberSlot"
                     template-key="sbxzjgq"
                    :disabled="true"
                    readonly-text="text"
                  />
        </template> -->
...
export default {
  ...
  data() {
    return {
      ...
      jiHua:[],
      dataConfig:{
        // "ndxzjhSGJSCX11":{t_ndsbxzjdjh:[]}, //设备检定/校准计划表
        // "yqsbxzrc":{t_yqsbxzjg:[]},       //设备检定/校准记录表
      ...
      listConfig: {
        ...
        columns: [
         /* { prop: 'subject', label: '任务标题', link: 'dialog', width: 250 }, */
          ...
          { prop: 'tUser', label: '确认人' , width: 80},
          { prop: 'endTime', label: '确认日期', sortable: 'custom', dateFormat: 'yyyy-MM-dd', width:110 },
        ],
        rowHandle: {
          actions: [/* {
  ...
  methods: {
     ...
     getConfigId(values,cont){
       ...
       curdPost('selects',this.packNameWhere('t_yqsbxzjg',wheres)).then(response => {
         let data = response.variables.data
                 for(let i in data){
                   for(let j in this.jiLiang){
                     if(this.jiLiang[j].bizKey == data[i].id_){
                       this.jiLiang[j]['facility']        = data[i].she_bei_ming_chen   //设备
                       this.jiLiang[j]['facilityFactor']  = data[i].jian_ding_xiao_zh   //机构名称
                       this.jiLiang[j]['facilityNumber']  = data[i].she_bei_bian_hao_   //设备编号
                       // console.log(data[i].she_bei_bian_hao_ )
                     }
                   }
                 }
    ...
    loadData() {
      ...
      queryPageList(this.getSearcFormData()).then(response => {
       ...
       // console.log('--data--',data)
          ...
          data[i]['month'] =data[i].endTime.slice(5,7)
              if(arr.length ==4){ //如果是年度计划， 标题编辑中取年份， 去除默认给与的年份
               ...
               if(data[i].formKey =='yqsbxzrc'){
                 this.jiLiang.push(data[i])
               }
               // if(data[i].formKey == 'ndxzjhSGJSCX11'){
               //   this.jiHua.push(data[i])
               // }


          }
          /* 新增*/
     if(this.jiLiang.length>0) {
         this.getConfigId(this.jiLiang,response.data)
     }else{

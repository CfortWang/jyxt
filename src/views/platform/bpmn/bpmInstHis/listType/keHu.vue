<template>
  <div>
    ...
    <template slot="firm"  slot-scope="scope">
          <ibps-custom-dialog
...
export default {
  ...
  data() {
    return {
      ...
      responseData:"",
      dataConfig:{
        // khmyddctj:{t_khmyddctj:[]}, //统计调查 myddc
        myddc:{t_myddc:[]},         //客户满意度调查表
      ...
      listConfig: {
        ...
        columns: [
         /* { prop: 'subject', label: '任务标题', link: 'dialog', width: 250 }, */
          ...
          { label: '客户单位名称' , width: 220, fieldType: 'slot',slotName:"firm"  },
          { prop: 'evaluator', label: '评价人' , width:100 },
          { prop: 'evaluationDate', label: '评价日期' , width: 150},
          { prop: 'tUser', label: '调查人' , width: 80},
          { prop: 'endTime', label: '调查时间', sortable: 'custom', dateFormat: 'yyyy-MM-dd', width:150 },
        ],
        rowHandle: {
          actions: [/* {
  ...
  methods: {
   ...
   getParenthesesStr(text) {
    ...
    // khmyddctj(cont,data){
    //    for(let i in data){
    //     if(data[i].bizKey ==cont.id_) data[i]["project"] = cont.ke_hu_
    //    }
    //  },
     /* 质控项目*/
     myddc(cont,data){
       // console.log(cont)
        for(let i in data){
         if(data[i].bizKey ==cont.id_){
            data[i]["project"] = cont.ke_hu_
            data[i]["type"] = cont.lei_bie_
            data[i]["evaluator"] = cont.ping_shen_ren_
            data[i]["evaluationDate"] = cont.ping_jia_shi_jian

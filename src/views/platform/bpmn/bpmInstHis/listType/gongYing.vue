...
export default {
  ...
  data() {
    return {
      ...
      jiLiang:[],
      dataConfig:{
        gyspjb:{t_gyspjb:[],}, //供应商评价
        gyskhb:{t_gyskh:[]}, //供应商考核
        gyssq:{t_gys:[]}//合格供应商
      },
      listConfig: {
        toolbars: [
        ...
        columns: [
          ...
          { prop: 'goodsFactory', label: '供应商', width: 120 },
  ...
  methods: {
   ...
   getParenthesesStr(text) {
    ...
    gyspjb(cont,data){
       for(let i in data){
        if(data[i].bizKey ==cont.id_) data[i]["goodsFactory"] = cont.gong_ying_shang_m
       }
        // resolve(true)
     },
     gyskhb(cont,data){ //供应商考核
        for(let i in data){
         if(data[i].bizKey ==cont.id_) data[i]["goodsFactory"] = cont.gong_ying_shang_m
        }
        // resolve(true)
      },
      gyssq(cont,data){ //供应商考核
        for(let i in data){
         if(data[i].bizKey ==cont.id_) data[i]["goodsFactory"] = cont.gong_ying_shang_
        }
        // resolve(true)
      },
    /**

<template>
  <div>
   ...
   <!-- <template slot="envirLocation"  slot-scope="scope">
         <ibps-custom-dialog
...
export default {
  ...
  data() {
    return {
      ...
      huanJing:[],
      dataConfig:{
        hjpg :{t_hjsbyqyl:[]},        //环境条件控制评估表
        hjnew:{t_hjsbyqyl:[]},        //设施环境条件控制一览表
      },
      listConfig: {
        toolbars: [
          { key: 'search' },
        ...
        columns: [
         /* { prop: 'subject', label: '任务标题', link: 'dialog', width: 250 }, */
          ...
          { prop: 'procDefName', label: '表单名称' , width: 180},
          { prop: 'location', label: '监控位置'  , width: 250,fieldType: 'slot',slotName:"bianZhiBuMenSelector" },

          // { prop: 'location', label: '监控位置'  , width: 250 },
  ...
  methods: {
   ...
   getParenthesesStr(text) {
    ...
    /* 各个设备表*/
    hjpg(cont,data){
      // console.log('环境条件控制评估表cont：',cont)
      // console.log('环境条件控制评估表data：',data)
       for(let i in data){
        if(data[i].bizKey ==cont.id_) data[i]["location"] = cont.bu_men_
       }
        // resolve(true)
     },
     hjnew(cont,data){
      // console.log('设施环境条件控制一览表cont：',cont)
      // console.log('设施环境条件控制一览表data：',data)
        for(let i in data){
         if(data[i].bizKey ==cont.id_) data[i]["location"] = cont.bu_men_
        }
        // resolve(true)

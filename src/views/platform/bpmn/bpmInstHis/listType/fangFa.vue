...
<!-- template-key="ffglwgl" 是下面原来的模板key -->
    <template slot="methodName"  slot-scope="scope">
          <ibps-custom-dialog
                :value="scope.row.methodName"
                 template-key="ffdhkwgl"
                :disabled="true"
                readonly-text="text"
              />
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      jiLiang:[],
      dataConfig:{
        "ffyxplsq"  : {t_ffyxplsqb:[]},          //方法允许偏离申请
        // "ffcxl"     : {t_ffcxfb:[]},             //标准方法查新记录表 不做 数据在子表 并且多个 无法直接渲染
        "bzffyzjlb" : {t_bzffyzjlb:[]},          //标准方法验证记录表
      ...
      listConfig: {
        ...
        columns: [
          ...
          { prop: 'tDept', label: '部门' , width: 60},
          { prop: 'procDefName', label: '表单名称',width:130 },
          { prop: 'methodName', label: '方法名称',  fieldType: 'slot',slotName:"methodName"  },
          { prop: 'tUser', label: '编制人' , width: 80},
          { prop: 'endTime', label: '完成时间', sortable: 'custom', dateFormat: 'yyyy-MM-dd', width:110 },
        ],
        rowHandle: {
          actions: [/* {
  ...
  methods: {
   ...
   loadData() {
     ...
     queryPageList(this.getSearcFormData()).then(response => {
      ...
      // console.log('--方法管理data--',data)
       ...
       // console.log('--方法管理dataConfig--',this.dataConfig)
         /* -0---------------*/
     }).catch(() => {
       this.loading = false
     })
   ...
   getParenthesesStr(text) {
    ...
    ffyxplsq(cont,data){        //方法允许偏离申请                  从数据库中对应
        // console.log('方法允许偏离申请',cont)
       for(let i in data){
        if(data[i].bizKey ==cont.id_) data[i]["methodName"] = cont.jian_ce_fang_fa_
       }
        // resolve(true)
     },
      bzffyzjlb(cont,data){      //标准方法验证记录表
        // console.log('标准方法验证记录表',cont)
        for(let i in data){
         if(data[i].bizKey ==cont.id_) {
           data[i]["methodName"] = cont.jian_ce_fang_fa_b
           // let method     = cont.jian_ce_fang_fa_b.split(',')
           // let method_len = method.length
           // console.log(method,' == ', method_len)
           // var finalMethodName  = ""
           // for(let x = 1;x<=method_len;x++){
              // finalMethodName+=(method[x-1])
              // console.log(method[x-1])
              // data[i]["methodName"] +=(method[x-1])
           // }
           // data[i]["methodName"] = finalMethodName
         }
        }
        // resolve(true)
      },

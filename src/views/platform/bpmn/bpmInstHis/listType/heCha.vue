<template>
  <div>
   ...
   <!-- <template slot="NumberSlot"  slot-scope="scope">
          <ibps-custom-dialog
...
export default {
  ...
  data() {
    return {
      ...
      jiLiang:[],
      dataConfig:{
        // bzwzhc:{t_bzwzqjhcjl:[],},        // 物料核查
        sbhcjl:{t_sbhcjlb:[]},            // 设备核查
        sbhc:{t_sbhcjh:[]},               // 年度设备核查计划
      },
      listConfig: {
        toolbars: [
          { key: 'search' },
        ...
        columns: [
         /* { prop: 'subject', label: '任务标题', link: 'dialog', width: 250 }, */
          ...
          { prop: 'number', label: '编号' , width: 80, width: 220 },
          { prop: 'facility', label: '设备名称' , width: 220,fieldType: 'slot',slotName:"facilitySlot" },
         // { prop: 'goods', label: '标准物料' , width: 220,fieldType: 'slot',slotName:"goodsSlot" },
          { prop: 'tUser', label: '核查人' , width: 80},
          { prop: 'endTime', label: '核查日期', sortable: 'custom', dateFormat: 'yyyy-MM-dd', width:110 },
        ],
        rowHandle: {
          actions: [/* {
  ...
  methods: {
   ...
   getParenthesesStr(text) {
    ...
    sbhcjl(cont,data){
     ...
     // bzwzhc(cont,data){ //物料
     //    for(let i in data){
     //     if(data[i].bizKey ==cont.id_) data[i]["goods"] = cont.biao_zhun_wu_zhi_
     //    }
     //  },

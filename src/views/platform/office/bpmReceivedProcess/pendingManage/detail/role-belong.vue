<template>
	<div :style="{height:height+'px'}" class="pendingManage-module">
		<ibps-crud ref="crud" :style="{ marginLeft: width+'px' }" :height="height" :data="listData"
			:toolbars="listConfig.toolbars" :search-form="listConfig.searchForm" :pk-key="pkKey"
			:columns="listConfig.columns" :pagination="pagination" :loading="loading" :index-row="false"
			@action-event="handleAction" @sort-change="handleSortChange" @column-link-click="handleLinkClick"
			@pagination-change="handlePaginationChange" :contorlLength="true">
		...
		<bpmn-formrender :visible="dialogFormVisible" :task-id="taskId" @callback="search"
				...
				listConfig: {
					...
					searchForm: {
						forms: [{
							...
							{
								prop: 'Q^proc_def_key_^S',
								name: 'Q^temp.proc_def_key_^S',
								label: '流程名称',
								fieldType: 'slot',
								slotName: 'procDefIdSelect',
								labelWidth: 80,
								itemWidth: 200
							},
							{

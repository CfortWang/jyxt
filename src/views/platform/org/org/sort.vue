...
export default {
  ...
  methods: {
    ...
    top() {
      ...
      const targetArr = data.filter(i => i.checked === true)
      targetArr.forEach(i => { targetIds.push(i.id) })
      this.formData = moveTop(data, targetIds, 'id')
    },
    // 向上
    up() {
      if (this.hasTip()) {
    ...
    buttom() {
      ...
      const targetArr = data.filter(i => i.checked === true)
      targetArr.forEach(i => { targetIds.push(i.id) })
      this.formData = moveButtom(data, targetIds, 'id')
    }
  }

  ...
  >
    ...
    >
      <el-form-item label="分级管理员用户:" prop="managerID">
        <ibps-employee-selector v-model="form.managerID" :disabled="!$utils.isEmpty(formId)" />
      </el-form-item>
  ...
  methods: {
    ...
    getFormData() {
      ...
      get({
        ...
        data.orgPerms = this.$utils.isEmpty(data.orgPerms) ? [] : data.orgPerms.split(',')
        data.userPerms = this.$utils.isEmpty(data.userPerms) ? [] : data.userPerms.split(',')
        this.form = data
        this.formValidate()
    ...
    saveData() {
      if (this.$utils.isEmpty(this.form.managerID)) {
        ActionUtils.warning('请选择分级管理员用户!')
        return
      }

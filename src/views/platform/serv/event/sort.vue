...
<script>
import { saveSort, queryPageList } from '@/api/platform/serv/event'
...
export default {
  ...
  methods: {
    loadData() {
      this.loading = true
      const params = ActionUtils.formatParams({ 'Q^SOURCE_ID_^S': this.sourceId }, null, null)
    ...
    // 提交保存数据
    saveData() {
      saveSort({ ids: this.ids.join(',') }).then(response => {

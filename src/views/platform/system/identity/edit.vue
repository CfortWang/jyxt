<template>
  <el-dialog
    ...
    :custom-class="!readonly?'is-fullscreen':''"
    width="60%"
    @open="getFormData"
    @close="closeDialog"
  >
...
export default {
  ...
  data() {
    ...
    const validateRule = (rule, value, callback) => {
      const regHaveOne = /((?=.*{NO})|(?=.*{no}))^.*?$/
      const regOnlyOne = /(?=.*{NO})(?=.*{no})^.*?$/
      const regNotCorrectNum = /{((?!((yy\b)|(yyyy\b)|(MM\b)|(M\b)|(dd\b)|(d\b)|(NO\b)|(no\b)))).*?}/g
      const regNotCorrecChar = /{yy[^a-zA-Z0-9}]+\w*}|{yyyy[^a-zA-Z0-9}]+\w*}|{MM[^a-zA-Z0-9}]+\w*}|{mm[^a-zA-Z0-9}]+\w*}|{dd[^a-zA-Z0-9}]+\w*}|{d[^a-zA-Z0-9}]+\w*}|{NO[^a-zA-Z0-9}]+\w*}|{no[^a-zA-Z0-9}]+\w*}/g
      if (!regHaveOne.test(value)) {
        callback(new Error('占位符{NO}或{no}至少有一个'))
      } else {
        if (!regOnlyOne.test(value)) {
          if (regNotCorrectNum.test(value) || regNotCorrecChar.test(value)) {
            let errorRegNum = value.match(regNotCorrectNum, regNotCorrecChar)
            let errorRegChar = value.match(regNotCorrecChar)
            if (errorRegNum == null) {
              errorRegNum = ''
            } else if (errorRegChar != null) {
              errorRegNum += ','
            } else {
              errorRegChar = ''
            }
            callback(new Error(errorRegNum + errorRegChar + ' 占位符不符合规则'))
          }
          callback()
        } else {
          callback(new Error('占位符{NO}和{no}不能同时存在'))
        }
      }
    }
    return {
  ...
  watch: {
    ...
    'identity.regulation': function(val) {
      const regYears = /{yyyy}/
      const regYear = /{yy}/
      const regMonths = /{MM}/
      const regMonth = /{M}/
      const regDays = /{dd}/
      const regDay = /{d}/
      if (regYears.test(val) || regYear.test(val)) {
        this.genTypeOptions[2].disabled = false
      } else {
        this.genTypeOptions[2].disabled = true
        this.identity.genType = 3
      }
      if (regMonths.test(val) || regMonth.test(val)) {
        this.genTypeOptions[1].disabled = false
      } else {
        this.genTypeOptions[1].disabled = true
        this.identity.genType = 3
      }
      if (regDays.test(val) || regDay.test(val)) {
        this.genTypeOptions[0].disabled = false
      } else {
        this.genTypeOptions[0].disabled = true
        this.identity.genType = 3
      }
    }
  },

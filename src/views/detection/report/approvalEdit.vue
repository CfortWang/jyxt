<template>
 ...
 <div class="form-header">
        <div class="title ibps-tc">尺寸检测报告审批
        </div>
      </div>
...
<script>
  ...
  import { selectById,saveSp,rejectedSp } from '@/api/detection/universalCRUD.js'
  export default {
    components: {
    ...
    methods: {
          ...
          let data={
            ...
            rejectedSp('sysbggl','rejectedsp',paramss).then(response => {
     ...
     // 提交
      confirmSubmit(){
        let data={
         ...
         saveSp('sysbggl','savesp',params).then(response => {
                 this.$emit('close', false)
                   this.$message("提交成功！")
                   this.$emit('loadData')
         })

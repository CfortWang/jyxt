<template>
 ...
 <div class="form-header">
        <div class="title ibps-tc">尺寸检测报告审核
        </div>
      </div>
...
<script>
  ...
  import { selectById,saveSh,rejectedSh } from '@/api/detection/universalCRUD.js'
  export default {
    components: {
    ...
    methods: {
          ...
          let data={
            ...
            rejectedSh('sysbggl','rejectedsh',paramss)
      ...
      confirmSubmit(){
        let data={
         ...
         saveSh('sysbggl','savesh',params).then(response => {
                 this.$emit('close', false)
                   this.$message("提交成功！")
                   this.$emit('loadData')
         })

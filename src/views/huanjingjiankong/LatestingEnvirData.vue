<template>
  <div>
    <div class="heads">实验室温湿度压差实验实时记录</div>
    <div class="top-box">
...
export default {
  ...
  created() {
    let this_= this;
    let sql =
      "select * FROM DATA_COPY WHERE deviceName IN ('HAU01','HAU02','HAU03','HAU04','PAU01','PAU02')  ORDER BY saveTime DESC LIMIT "+this.limit;
    this.currentSql = sql;
    this.loadData(sql);
    let sql1 = "select COUNT(*) as COUNT FROM DATA_COPY "
  ...
  methods: {
    ...
    filter() {
      ...
      let sql = "";
      if (quyu && riqi) {
         sql = "select * FROM DATA_COPY WHERE deviceName = '" + quyu +"' and saveTime like '%" +riqi +"%'  ORDER BY saveTime DESC LIMIT "+this.limit;
         this.countSql ="select COUNT(*) as COUNT FROM DATA_COPY WHERE deviceName = '" + quyu +"' and saveTime like '%" +riqi +"%'  ORDER BY saveTime DESC LIMIT "+this.limit;
         this.countLength(this.countSql);
      } else if (quyu && !riqi) {
        sql ="select * FROM DATA_COPY WHERE deviceName = '" +quyu +"' ORDER BY saveTime DESC LIMIT "+this.limit;
        this.countSql ="select COUNT(*) as COUNT FROM DATA_COPY WHERE deviceName = '" + quyu +"' ORDER BY saveTime DESC LIMIT "+this.limit;
        this.countLength(this.countSql);
      } else if (!quyu && riqi) {
       sql = "select * FROM DATA_COPY WHERE  saveTime like '%" +riqi +"%'  ORDER BY saveTime DESC LIMIT "+this.limit;
       this.countSql ="select COUNT(*) as COUNT FROM DATA_COPY WHERE  saveTime like '%" +riqi +"%'  ORDER BY saveTime DESC LIMIT "+this.limit;
       this.countLength(this.countSql);
      }else if (!quyu && !riqi) {
       sql =  "select * FROM DATA_COPY WHERE deviceName IN ('HAU01','HAU02','HAU03','HAU04','PAU01','PAU02')  ORDER BY saveTime DESC LIMIT "+this.limit;
       this.countSql = "select COUNT(*) as COUNT FROM DATA_COPY ";
       this.countLength(this.countSql);
      }
      return sql;
    },

<template>
	<div class="container-component">
		...
		<div ref="ibpsCrud" class="ibps-container-crud">
			...
			<!-- 工具栏 -->
			<!-- 标题栏-->
			<div v-if="displayField && displayField!='N'&& displayField!='true'&& displayField!='Y'"
				class="jbd-title-cont">
				{{displayField}}
			</div>
			<div v-if="toolbars || $slots.header" ref="ibpsCrudHeader" class="ibps-container-crud__header">
				<div v-if="toolbars" :class="['ibps-toolbar--' +$ELEMENT.size]" class="ibps-toolbar">
					<div class="header" style="overflow:scroll;
          height: 100%;
          ...
          overflow: hidden">
						<!-- 换了位置 -->
						<div v-show="$utils.isNotEmpty(searchForm) && showToolbar" ref="toolbarBox"
							class="toolbar-box clear" style="padding: 0px; border: 0px;">
							<template v-if="$slots.searchForm" ref="searchForm">
								<slot :loading="loading" name="searchForm" />
							</template>

							<template v-else>
								<search-form v-if="searchForm" ref="searchForm" :forms="searchForm.forms||[]"
									size="mini" :fuzzy="searchForm.fuzzy" :inline="searchForm.inline"
									:label-width="searchForm.labelWidth" :item-width="searchForm.itemWidth"
									...
									<template slot="jbd-buttons" v-if="searchFormSlot.length>0">
										<ibps-toolbar :actions="toolbars" :socpe="thatSocpe"
			...
			<!--列表-->
			<!--   @header-dragend="handleHeaderDragend" 拖拽事件取消 -->
			<el-table ref="elTable" v-loading="loading" v-bind="options" :row-key="pkKey" :data="ibpsData"
				:height="tableHeight" :header-cell-style="{color:'#000','font-size':'14px',padding:'6px 6px'}"
				:stripe="false" :row-class-name="tableRowClassName" @current-change="handleCurrentChange"
				@select="handleSelect" @select-all="handleSelectAll" @selection-change="handleSelectionChange"
				@sort-change="handleSortChange" @cell-mouse-enter="handleCellMouseEnter"
				...
				<!--选择列 多选-->
				<el-table-column v-if="(selectionRow || selectionRow === '') && selectionType === 'checkbox'"
					v-bind="typeof selectionRow === 'Object'?selectionRow:null"
					:label="handleAttribute(selectionRow.label, '')"
					:reserve-selection="selectionRow.reserveSelectione||false" type="selection" show-overflow-tooltip />
				<!--选择列 单选-->
				<el-table-column v-if="(selectionRow || selectionRow === '') && selectionType === 'radio'"
					:label="selectionRow.label||''" :width="selectionRow.width||55" show-overflow-tooltip>
				...
				<!-- 自定义表单名称-->
				<el-table-column :width="150" show-overflow-tooltip label="表单名称" v-if="formName">
					<template slot-scope="scope">
						{{formName}}
					</template>
				</el-table-column>
				...
				<!--索引列-->
				<el-table-column v-if="indexRow || indexRow === ''" v-bind="typeof indexRow === 'Object'?indexRow:null"
					:label="handleAttribute(indexRow.label, $t('components.crud.index'))" :width="60" type="index" />
				<slot name="prepend-column" />
				<template v-for="(column, index) in tableColumns">
					<el-table-column v-if="showColumn(column)" :key="index" :prop="handleAttribute(column.prop, null)"
						:label="handleAttribute(column.label, '')" :width="handleAttribute(column.width,null)"
						v-bind="column" show-overflow-tooltip>
						...
						<template slot-scope="scope">
							...
							<span v-else-if="column.options" class="ibps-table-column">
								...
								<template v-if="column.dataType==='stringArray'">
									<span
										...
										:key="i">
										{{ handleColumnOptions('label',column.options,value,column) }}<template
											v-if="handleColumnDataConvert(scope.row[column.prop],column.separator).length-1 !==i">,</template>
									</span>
								</template>
								...
								<span
									v-else>{{ handleColumnOptions('label',column.options,scope.row[column.prop]) }}</span>

							</span>
							<!-- tags组件-->
							<span v-else-if="column.tags" class="ibps-table-column">
								<span
									v-if="$utils.isEmpty(scope.row[column.prop]) ">{{ column.defaultLabel||'' }}</span>
								<!--stringArray 字符串类型的数组-->
								<template v-else-if="column.dataType==='stringArray'">
									<el-tag
										v-for="(value,i) in handleColumnDataConvert(scope.row[column.prop],column.separator)"
										:key="i" :type="handleColumnOptions('type',column.tags,value,column)">
							...
							<span v-else-if="column.link" class="ibps-table-column">
								<el-link :type="column.type||'primary'" :underline="column.underline||false"
							...
							<span v-else-if="column.slotName" class="ibps-table-column">
								<slot :name="column.slotName" :row="scope.row" :value="scope.row[column.prop]"
									:column="column" :$index="scope.$index" />
							</span>

							<!--自定义组件-->
							<render-custom-component v-else-if="column.component && column.component.name"
								v-model="scope.row[column.prop]" :component-name="column.component.name" />
							<!--渲染组件-->
							<render-component v-else-if="column.component && column.component.render"
								:render-function="column.component.render" :scope="scope" />
							<template v-else>
						...
						<template v-if="column.children">
							<el-table-column show-overflow-tooltip v-for="(column2, index2) in column.children"
								:key="index2" :label="handleAttribute(column2.title, '')"
								:prop="handleAttribute(column2.key, null)" v-bind="column2">
								<template slot-scope="scope">
									<!--过滤器-->
									...
									<span v-else-if="column2.tags" class="table-column-tags">
										<el-tag :type="handleTags('type',column2.tags,scope.row[column2.prop])">
											{{ handleTags('label',column2.tags,scope.row[column2.prop]) }}</el-tag>
									</span>
									<!-- link组件-->
									<span v-else-if="column2.link">
										<a href="javascript:void(0)" class="table-column-link"
											@click="handleColumnLink(column2,scope.row)">{{ scope.row[column2.prop] }}</a>
									</span>
									<span v-else-if="column2.slotName">
										<slot :name="column2.slotName" :row="scope.row" :$index="scope.$index" />
									</span>
									<!--自定义组件-->
									<render-custom-component v-else-if="column2.component && column2.component.name"
										v-model="scope.row[column2.prop]" :component-name="column2.component.name" />
									<!--渲染组件-->
									<render-component v-else-if="column2.component && column2.component.render"
										:render-function="column2.component.render" :scope="scope" />
									<span v-if="contorlLength">
										<el-tooltip class="item" effect="dark"
											:content="column.formatter ? column.formatter(scope.row, scope.column, _get(scope.row, column.prop), scope.$index) : _get(scope.row, column.prop)"
											placement="bottom">
											<span>
									...
									<template
										v-else>{{ column2.formatter ? column2.formatter(scope.row, scope.column2, _get(scope.row, column2.prop), scope.$index) : _get(scope.row, column2.prop) }}</template>
								</template>
							</el-table-column>
							<!-- end 子列-->
						</template>
				...
				<el-table-column v-if="rowHandle" v-bind="rowHandle"
					...
					<template v-slot:default="scope">
						...
						<template v-if="hasRowHandleActions(scope.row)">
							<template v-if="rowHandle.effect ==='display'">
								<ibps-toolbar v-if="!rowHandleMoreActions || rowHandleMoreActions.length <=1"
									:actions="rowHandleActions" :socpe="thatSocpe" :data="scope.row" position="manage"
									type="link" @action-event="handleActionEvent" />
								<template v-if="rowHandleMoreActions && rowHandleMoreActions.length <=1">
									<div class="el-divider el-divider--vertical" />
									<ibps-toolbar :actions="rowHandleMoreActions" :socpe="thatSocpe" :data="scope.row"
										position="manage" type="link" @action-event="handleActionEvent" />
								</template>
								...
								<template v-if="rowHandleMoreActions && rowHandleMoreActions.length >1">
									<el-dropdown>
										...
										<el-dropdown-menu slot="dropdown" class="ibps-table-dropdown-menu">
											<ibps-toolbar :actions="rowHandleMoreActions.concat(rowHandleActions)"
												:socpe="thatSocpe" :data="scope.row" position="manage" type="linkHide"
												@action-event="handleActionEvent" />
										</el-dropdown-menu>
									</el-dropdown>
								</template>
							...
							<template v-else>
								<el-dropdown>
									...
									<el-dropdown-menu slot="dropdown" class="ibps-table-dropdown-menu">
										<ibps-toolbar :actions="rowHandleActions" :socpe="thatSocpe" :data="scope.row"
											position="manage" @action-event="handleActionEvent" />
									</el-dropdown-menu>
								</el-dropdown>
			...
			<div v-if="isShowPagination" ref="ibpsCrudFooter" class="ibps-container-crud__footer">
				...
				<el-pagination v-else :current-page="currentPage" :page-size="pageSize"
					:page-count="pagination[pageCountKey]" :total="pagination[totalKey]" v-bind="paginationOptions"
					@size-change="handlePaginationSizeChange" @current-change="handlePaginationCurrentChange"
					@prev-click="handlePaginationPrevClick" @next-click="handlePaginationNextClick">
					<template>
		...
		<!--显示字段-->
		<display-field-dialog :visible="displayFieldVisible" :fields="columns" :data="displayFields"
			@callback="handleDisplayField" @close="visible => displayFieldVisible = visible" />
	</div>
</template>

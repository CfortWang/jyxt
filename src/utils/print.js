/**
 * 打印类属性、方法定义
 * 参考：https://github.com/xyl66/vuePlugs_printjs/blob/master/print.js
 *
 * */
...
Print.prototype = {
  ...
  getStyle: function() {
    ...
    str += '<style>' + (this.options.noPrint ? this.options.noPrint : '.no-print,.hidden-print') + '{display:none;}</style>'
  ...
  writeIframe: function(content) {
    ...
    var f = document.getElementsByTagName('body')[0].appendChild(iframe)
    iframe.id = 'myIframe'
    ...
    const _this = this
    const load = function() {
      _this.toPrintIframe(w, iframe)
    }
    if (iframe.addEventListener) { // IE8以上浏览器支持
      iframe.addEventListener('load', load(), true)
    } else if (iframe.attachEvent) { // IE8及以下浏览器支持
      // IE8及以下浏览器需要加on
  ...
  toPrintIframe: function(frameWindow, iframe) {

...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      // BUG--3316
      const nodeUsers = []
      const outgoingNodesUsers = this.nodesUsersMap || {}
      const selectNodeUser = this.selectNodeUser
      for (var o in outgoingNodesUsers) {
        const executors = []
        for (var s in selectNodeUser) {
          outgoingNodesUsers[o].forEach(user => {
            if (selectNodeUser[s].indexOf(user.id) > -1 && o === s) {
              user.groupType = ''
              executors.push(user)
            }
          })

<template>
  ...
  >
        ...
        <tr v-for="({nodeId,name}) in outgoingNodes" :key="nodeId">
          ...
          <td>
            ...
            <el-checkbox
              ...
              size="mini"
              :disabled="$utils.isEmpty(outgoingNodesUsersMap[nodeId])"
    ...
    <div slot="footer" class="el-dialog--center">
      <el-button type="primary" icon="ibps-icon-send" @click="handleSave()">{{ actionTitle }}</el-button>
  ...
  methods: {
    getFormData() {
      ...
      startOutgoing({
        ...
        this.outgoingNodesUsersMap = variables.outgoingNodesUsersMapObject
    ...
    handleSelectorActionEvent(buttonKey, data) {
      if (buttonKey === 'confirm') {
        var chenck = Object.prototype.toString
        let outgoingNodesUsersMap
        if (chenck.call(this.outgoingNodesUsersMap) === '[object String]') {
          outgoingNodesUsersMap = JSON.parse(this.outgoingNodesUsersMap)
        } else {
          outgoingNodesUsersMap = this.outgoingNodesUsersMap
        }
        const nodeUsers = outgoingNodesUsersMap[this.curNodeId]
        const ids = []
        ...
        this.outgoingNodesUsersMap = outgoingNodesUsersMap

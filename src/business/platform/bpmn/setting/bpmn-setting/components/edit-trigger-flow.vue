      ...
      <el-row>
        <el-col :span="10">
          <el-card class="box-card">
            <div slot="header" class="clearfix">
              <span>源数据信息</span>
              <el-button type="text" @click.native="getSrcReadData(true)">数据源加载</el-button>
            </div>
            <draggable
              :list="srcReadList"
        ...
        <el-col :span="14">
          <el-card class="box-card">
            <div slot="header" class="clearfix">
              <span>目标数据信息</span>
              <el-button type="text" @click.native="getDestReadData(true)">数据源加载</el-button>
            </div>
...
export default {
  ...
  methods: {
    ...
    dragstart({ newIndex, oldIndex }) {
      this.dragData = this.srcReadList[oldIndex]
    },
    // 拖拽结束
    dragend(event) {
    ...
    handleSave() {
      if (this.$utils.isEmpty(this.form.trigerFlowKey)) {
        ActionUtils.warning('请绑定触发流程！')
        return
      }
      if (this.form.bpmTrigerParamPoList.length === 0) {
        ActionUtils.warning('未绑定目标数据信息！')
        return
      }
      if (this.bpmn.defKey === this.form.trigerFlowKey) {
        this.$message({
          message: '触发流程设定不可设置本身，请重新选择流程信息！',
          type: 'warning'
        })
        return
      }
    ...
    getFormData() {
      ...
      if (this.$utils.isNotEmpty(this.boData) &&
       this.$utils.isNotEmpty(this.boData.code)) {
        this.getSrcReadData()
      }
    },
    getActionName() {
    ...
    // 获取目标数据
    getDestReadData(init) {
      if (this.$utils.isEmpty(this.form.trigerFlowKey)) {
        ActionUtils.warning('请选择触发流程')
        return
      }
      ...
      getDestRead({ defKey: this.form.trigerFlowKey }).then(response => {
        ...
        if (this.$utils.isEmpty(attrs)) {
          ActionUtils.warning('目标数据未配置业务对象！')
          return
        }

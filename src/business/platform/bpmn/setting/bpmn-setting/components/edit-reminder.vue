<template>
  ...
  >
      ...
      <el-tabs v-model="activeIndex" type="card">
        ...
        <el-tab-pane label="发送催办消息设置" name="3">
          <div>
            ...
            <el-col :span="11">
              <el-form-item v-if="isSupervise === 'Y'" label="开始督办次数:" prop="triggerRemindCount">
                <el-select v-model="triggerRemindCount" placeholder="请选择">
                  <el-option
                    v-for="item in timesOptions"
                    :key="item.value"
                    :label="item.label"
            ...
            <el-col :span="9" :offset="1">
              <el-button v-if="isSupervise === 'Y'" type="primary" @click="handleSupervise">设置</el-button>
            </el-col>
          </div>
        </el-tab-pane>
        <!-- 消息内容 -->
    ...
    />
    <supervise-dialog
      :visible="superviseSettingVisible"
      :data="form.supervise"
      :cur-node="curNode"
      style="width:'100%'"
      class="form-content"
      @callback="handleSuperviseData"
      @close="visible => superviseSettingVisible = visible"
    />
  </el-dialog>
</template>
...
import superviseDialog from './edit-supervise'
...
export default {
  ...
  methods: {
    ...
    changeIsSupervise(val) {
      if (this.isSupervise === 'N') {
        this.form.supervise = {}
      } else if (this.isSupervise === 'Y') {
        this.form.supervise = {
          procDefId: '',
          name: '',
          relTimeType: 'caltime',
    ...
    handleSave() {
      ...
      if (this.isSupervise === 'Y') {
        this.form.supervise.triggerRemindCount = this.triggerRemindCount
      }
    ...
    getFormData() {
      ...
      if (this.$utils.isNotEmpty(this.form.supervise)) {
        if (this.$utils.isNotEmpty(this.form.supervise.name)) {
          this.isSupervise = 'Y'
          this.triggerRemindCount = this.form.supervise.triggerRemindCount
        } else {
          this.isSupervise = 'N'
        }
      }
    ...
    handleSupervise() {
      this.superviseSettingVisible = true
    },
    handleSuperviseData(val) {
      this.form.supervise = val
    }
  }

<template>
  <div class="panel-body">
    <el-form ref="formSet" v-model="form" label-width="100px" @submit.native.prevent>
      ...
      <el-form-item label="表单：" prop="formValue">
        <form-def-selector
          ...
          :temporary-data="form.editFormRights||{}"
      ...
      >
        <el-input
          v-model="form.mobileValue"
          placeholder="移动表单URL"
        />

      </el-form-item>
      <el-form-item
        v-if="form.type=='inner'"
        label="打印模版："
        prop="formPrint"
      >
        <form-print-selector
          ...
          :form-key="formKey"
          :disabled="$utils.isEmpty(form.formValue)"
        />
      </el-form-item>

      <template v-if="formType==='flow' || formType==='node' ">
        <el-form-item label="前置处理器：" prop="prevHandler">
          <el-input v-model="attributes.prevHandler" placeholder="前置处理器" type="textarea" rows="2" autosize />
        </el-form-item>
        <el-form-item label="后置处理器：" prop="postHandler">
          <el-input v-model="attributes.postHandler" placeholder="后置处理器" type="textarea" rows="2" autosize />
        </el-form-item>
      </template>
    </el-form>
    <!-- 表单意见-->
